...
void waitForVBlank(void) {
  ...
  // Finally, increment the vBlank counter:
    vBlankCounter++;//jankauski check it if broken.
}
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image, int selector) {
  if(selector == 0){
    for(int i = 0; i < height;i++){//classic
      DMA[3].dst = &videoBuffer[OFFSET(row+i, col, 240)];
  ...
  if(selector == 1){
    for(int i = 0; i < height;i++){//special
      DMA[3].dst = &videoBuffer[OFFSET(row+i, col, 240)];
      DMA[3].src = &image[col + (row+i)*240];
      DMA[3].cnt = (width) | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT | DMA_ON;
    //UNUSED(dumper);
    }
  }

...
static metadata_t *find_right(metadata_t *freed_block){
    ...
    while (curr) {
        if ((uintptr_t) (curr) == (uintptr_t) freed_block+freed_block->size+TOTAL_METADATA_SIZE) {
            return curr;
        }
        curr = curr->next;
    }
...
static metadata_t *split_block(metadata_t *block, size_t size){
    ...
    metadata_t *newAddress = (metadata_t*)((uintptr_t)block + TOTAL_METADATA_SIZE + block->size-newSize);//bruh dingus forgot to cast...
    block->size = block->size-newSize;
    newAddress->size = size;
    return newAddress;
}
...
static void add_to_addr_list(metadata_t *block){
    ...
    metadata_t *currAddress = address_list;
    //set to curr to address
    metadata_t *holder = NULL;//temp var.
    while(currAddress && currAddress<block){
        holder = currAddress;
        currAddress = currAddress->next;
    }
    block->next = holder->next;
    holder->next = block;
}
...
static metadata_t *find_best_fit(size_t size){
    metadata_t *curr = address_list;
    metadata_t *nextFit = NULL;
    while(curr){
        if(curr->size == size)return curr;
        if(size<(curr->size)){
            if(!nextFit||(nextFit->size)>(curr->size)){
                nextFit = curr;
            }
        }
        curr = curr->next;
...
void *my_malloc(size_t size) {
    ...
    metadata_t *myBlock = find_best_fit(size);
    ...
    if(!myBlock){
        ...
        if(newBlock == (void*)-1){
            ...
            return NULL;//out of mem? check it...
        }
        ...
        newBlock->next = NULL;
        my_free((void*)((uintptr_t)newBlock+TOTAL_METADATA_SIZE));
    ...
    // 5. If the block is big enough to split:
        if(myBlock->size<size+MIN_BLOCK_SIZE){
            remove_from_addr_list(myBlock);
            return (void*)((uintptr_t)myBlock+TOTAL_METADATA_SIZE);
        }
    ...
    // A lot of these steps can be simarrauplified by implementing helper functions. We highly recommend doing this!


    return (void*)(TOTAL_METADATA_SIZE+(uintptr_t)split_block(myBlock,size));
}

/* FREE
 * See PDF for documentation
 */
void my_free(void *ptr) {//janker no work...
    my_malloc_errno = NO_ERROR;
    if(!ptr)return;
    ...
    metadata_t *startPoint = (metadata_t*)((uintptr_t)ptr - TOTAL_METADATA_SIZE);
    metadata_t *secondPoint = find_left(startPoint);//higher up so damage doesn't occur when pulling vals.
    if(find_right(startPoint)){
        //printf("yolo woo you there??");
        metadata_t *holder = find_right(startPoint);
        remove_from_addr_list(find_right(startPoint));
        merge(startPoint,holder); 
    }
    if(secondPoint){
        //printf("yolo wo2o 2you there??");
        remove_from_addr_list(secondPoint);
        merge(secondPoint,startPoint);
    ...
    add_to_addr_list(startPoint);
    /*grr func used are
    add_to_addr_list
    remove_from_addr_list   - Given by vry kind TAs... no errors hopefully 
    merge                   - Unlikely to be jank.
    find_right
    find_left               - Given by vry kind TAs... no errors hopefully 
    */
}
...
void *my_realloc(void *ptr, size_t size) {
    ...
    metadata_t *startPoint = my_malloc(size);
    if(!startPoint)return NULL;
    size_t prevSize =((metadata_t*)((uintptr_t)ptr-TOTAL_METADATA_SIZE))->size;
    if(size<prevSize)size = prevSize;
    memcpy(startPoint,ptr,size);
    my_free(ptr);
    return startPoint;
...
void *my_calloc(size_t nmemb, size_t size) {
    my_malloc_errno = NO_ERROR;
    metadata_t *startPoint = my_malloc(nmemb*size);
    if(!startPoint)return NULL;
    memset(startPoint, '\0',nmemb*size);

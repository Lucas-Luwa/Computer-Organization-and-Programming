...
;;setup
    ...
    ;ADD R6, R6, -8  ; allocate space to save all five registers R0-R4
...
WHILE   LDR R3, R5, #4       ; array initialization ; again just in case we have some funny business happening
    ...
    WHILE2  LDR R4, R5, #5   ; len = len;
            NOT R4, R4      ; FLIP BITS
            ADD R4, R4, #1  ; len = -len
            ADD R4, R4, R2  ; k-len < 0 keep goin'
            BRzp ENDGAME

                ADD R3, R3, R2  ; ARRAY[j]
...
ENDGAME      
    ...
    LDR R4, R4, #0  ; Value Set R4 = temp = ARRAY[i]

    LDR R3, R5, 4   ; array initialization
    AND R2, R2, 0   ; Clear R2
    ADD R2, R3, R1  ; ARRAY[j]
    LDR R2, R2, #0  ; Value set R2 for ARRAY[j] aka k destroyed.
